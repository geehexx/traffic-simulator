...
bazel build //... --config=remote --remote_header=x-buildbuddy-api-key=***REMOVED***
...
2. **Set Environment Variable**: Add to your shell profile
   ...
   source ~/.bashrc
   ```
3. **Test Connection**: `bazel build //... --config=remote --remote_header=x-buildbuddy-api-key=***REMOVED***
