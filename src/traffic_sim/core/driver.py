...
class Driver:
    ...
    def _update_speeding_rates(self) -> None:
        ...
        aggression_factor = max(0.0, self.params.aggression_z)  # Only positive aggression increases speeding
...
def _gaussian_copula_sample(
    ...
    try:
        ...
        cov_matrix = [[correlation_matrix[i][j] * stds[i] * stds[j] 
...
def sample_driver_params(cfg: Dict[str, Any], rng: random.Random) -> DriverParams:
    ...
    param_names = [
        "reaction_time_s", "headway_T_s", "comfort_brake_mps2", "max_brake_mps2",
        "jerk_limit_mps3", "throttle_lag_s", "brake_lag_s", "aggression_z", "rule_adherence_z"
    ]
    ...
    corr_matrix[0][2] = corr_matrix[2][0] = correlations.get("A_b_comf", 0.3)  # Aggression vs Comfort braking
    corr_matrix[0][8] = corr_matrix[8][0] = correlations.get("R_A", -0.4)  # Rule adherence vs Aggression
    corr_matrix[0][0] = correlations.get("distraction_t_reaction", 0.5)  # Distraction vs Reaction time
