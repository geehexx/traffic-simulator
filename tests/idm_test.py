...
def test_speeding_behavior():
    ...
    assert (
        speeding_events > 50
    ), f"Expected aggressive driver to speed more often, got {speeding_events} events"
...
def test_jerk_limiting():
    ...
    assert (
        abs(actual_change) <= expected_max_change + 1e-6
    ), f"Jerk limit exceeded: {abs(actual_change)} > {expected_max_change}"
...
def test_enhanced_idm_stability():
    ...
    for vehicle in sim.vehicles:
        ...
        assert (
            -10.0 <= vehicle.state.a_mps2 <= 5.0
        ), f"Acceleration out of range: {vehicle.state.a_mps2}"
        assert (
            abs(vehicle.internal.jerk_mps3) <= vehicle.driver.params.jerk_limit_mps3 + 1e-6
        ), f"Jerk limit exceeded: {vehicle.internal.jerk_mps3}"
...
def test_correlation_sampling():
    ...
    numerator = sum(
        (a - mean_aggression) * (h - mean_headway)
        for a, h in zip(aggression_values, headway_values)
    )
    denominator = (
        sum((a - mean_aggression) ** 2 for a in aggression_values)
        * sum((h - mean_headway) ** 2 for h in headway_values)
    ) ** 0.5
...
def test_ssd_safety_margin():
    ...
    assert (
        abs(ssd - expected_ssd) < 1e-6
    ), f"SSD should include safety margin: {ssd} vs {expected_ssd}"
